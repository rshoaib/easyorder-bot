...
export default async function MenuPage({ params }: { params: { slug: string } }) {
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6">
            ...
            {products.length === 0 ? (
                /* Enhanced empty state with setup help offer */
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    <SetupHelpBanner variant="empty-menu" />
                    <div className="border-t border-slate-100 p-4 text-center">
                        <Link href={`/admin/${slug}/menu/new`}>
                            <button className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 transition-colors mx-auto">
                                <Plus size={20} />
                                Or Add Products Yourself
                            </button>
                        </Link>
                    </div>
                </div>
